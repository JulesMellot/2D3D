"""
Script to process the full video with optimization for high-resolution videos.
"""
...
def process_full_video(input_path, output_path, profile="fast", max_dimension=1920):
    """
    Process the full video with optimizations for high-resolution videos.
    ...
    try:
        ...
        # Create side-by-side stereo video
        ...
        # Use ffmpeg to create stereo video
        os.system(f"ffmpeg -y -framerate {fps} -i {left_pattern} -framerate {fps} -i {right_pattern} "
                  f"-filter_complex '[0:v][1:v]hstack=inputs=2[v]' -map '[v]' -map 0:a? "
                  f"-c:v libx264 -preset medium -crf 23 -pix_fmt yuv420p {output_path}")
...
def main():
    ...
    parser.add_argument("-p", "--profile", choices=["fast", "balanced", "precision"], 
                        default="fast", help="Depth estimation profile")
    ...
    try:
        process_full_video(args.input, output_path, args.profile, args.max_dimension)
